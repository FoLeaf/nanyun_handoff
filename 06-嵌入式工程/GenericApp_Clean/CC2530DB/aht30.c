...
static uint8_t AHT30_CheckStatus(AHT30_HandleTypeDef *dev)
{
    uint8_t status = 0;
...
uint8_t AHT30_Init(AHT30_HandleTypeDef *dev)
{
    ...
    uint8_t status = AHT30_CheckStatus(dev);
...
{
    uint8_t meas_cmd[3] = {AHT30_CMD_MEASURE, 0x33, 0x00};
    uint8_t read_buf[7];
    ...
    // Parse data
    raw_humi = ((uint32_t)read_buf[1] << 12) | ((uint32_t)read_buf[2] << 4) | ((uint32_t)read_buf[3] >> 4);
    raw_temp = (((uint32_t)(read_buf[3] & 0x0F)) << 16) | ((uint32_t)read_buf[4] << 8) | (uint32_t)read_buf[5];
